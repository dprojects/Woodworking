...
#
# Supported locales on FreeCAD <2024-01-20, `FreeCADGui.supportedLocales()`, total=43>:
...
# - Use `pylupdate6` instead of default `pylupdate5`
# 	$ PYLUPDATE=pylupdate6 ./update-translations.sh -u es-ES
#
...
_lupdate="${LUPDATE:-lupdate}"
_lconvert="${LCONVERT:-lconvert}"
_lrelease="${LRELEASE:-lrelease}"
_pylupdate="${PYLUPDATE:-pylupdate5}"
...
get_strings() {
	...
	# _lupdate ../ui/*.ui -ts uifiles.ts -no-obsolete
	# Get translatable strings from Python file(s)
	$_pylupdate -no-obsolete -ts pyfiles.ts ../{InitGui,loadMenu,loadTools}.py \
		../Tools/{,MagicPanels/}*.py
	# Join strings from Qt Designer and Python files into a single temp file
	$_lconvert -i pyfiles.ts -o _${WB}.ts -sort-contexts -no-obsolete
}
...
update_locale() {
	...
	if [ ! -f "${WB}${u}${locale}.ts" ]; then
		...
		get_strings
		if [ "$locale" == "" ]; then
			$_lconvert -i _${WB}.ts -o ${WB}.ts
		else
			$_lconvert -source-language en -target-language "${locale//-/_}" \
		...
		get_strings
		if [ "$locale" == "" ]; then
			$_lconvert -i _${WB}.ts ${WB}.ts -o ${WB}.ts
		else
			$_lconvert -source-language en -target-language "${locale//-/_}" \
