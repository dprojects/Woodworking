...
| Name | FreeCAD code | Google AI code |
| :--- | :--: | :--: |
| Afrikaans | af | af |
| Arabic | ar | ar |
| Basque | eu | eu |
...
* Furniture face frame ( vertical, for custom changes )
* Furniture face frame ( horizontal, around )
* Furniture face frame ( horizontal, with center )
* Furniture face frame ( horizontal, for custom changes )
...
* Furniture front outside
* Furniture front outside ( decorative )
* Furniture front outside with glass ( simple frame )
* Furniture front outside with glass ( frame with decoration )
* Furniture front inside
* Furniture front inside ( decorative )
* Furniture front inside with glass ( simple frame )
* Furniture front inside with glass ( frame with decoration )
* Furniture front decoration ( simple frame )
* Furniture front left (decoration, import parametric )
* Furniture front right (decoration, import parametric )
...
* Furniture top (decoration, import parametric )
...
* Furniture side
* Furniture center board
* Furniture side decoration ( simple frame )
* Furniture sides with holes ( import parametric )
...
* Furniture back board ( outside, HDF )
* Furniture back board ( inside, full )
...
* Cabinet foot ( good for cleaning )
* Cabinet foot ( standard )
* Cabinet foot ( more stable )
* Cabinet foot ( decorated )
* Cabinet foot ( chair style )
...
* Wooden table ( kitchen simple style )
* Wooden table ( kitchen modern style )
* Wooden table ( kitchen decorated style )
* Wooden table ( coffee simple style )
* Wooden table ( coffee modern style )
* Wooden table ( coffee decorated style )
...
* Shelf support 5x16 mm ( import parametric )
* Handle ( single hole )
* Handle ( single hole, decorated )
* Handle ( double hole )
* Handle ( double hole, decorated )
* Hooks for wall cabinets ( [Camar 807](https://www.camar.it/prodotti_scheda.php?cat_id=31&prod_id=807%2002%20Z1%20IN%20__&id=259) ) 
* Angle supporter 30x30x25 mm ( import parametric )
* Angle supporter 80x80x20 mm ( import parametric )
* Angle supporter 40x40x100 mm ( import parametric )
...
* **Move:** In this mode you can move any object with custom step. You can also move containers, for example panel with dowels inside `LinkGroup`.
  ...
  * `Step to move` if the object is recognized the `Step to move` will be set by default with selected object thickness. The offset is calculated from objects anchors, usually left bottom corner (vertex).
...
* **Cross:**
  ...
  * `Center cross:` buttons `Turn on`, `Turn off` turn on and off the center cross at the screen.
...
* **Width of transport space:** This is the width of the cargo space, for example, the width of the trunk in a car. By default, it is set to the width of my new Kia Picanto. This value is used by [getDimensions](#getdimensions) cut-list tool to create `transport space limit` report type.
* **Length of transport space:** This is the length of the cargo space, for example, the length of the trunk of a car towards the front of the car. By default, it is set to the length of the trunk space in my new Kia Picanto with the seats folded down. This value is used by [getDimensions](#getdimensions) cut-list tool to create `transport space limit` report type.
...
* **Lumber allowance for thickness:** this option allows you to set the wood allowance for the board's thickness; it applies to the board's shortest edge. This value allows you to leave a margin of wood for later planing or cutting to achieve the final dimensions. However, in the case of the [r - raw wood](https://github.com/dprojects/Woodworking/tree/master/Docs#main-report) cutting list, the thickness will be rounded to the first threshold encountered from the global variable `MagicPanels.gSawmillSteps`, which can be set using the [magicSettings](https://github.com/dprojects/Woodworking/tree/master/Docs#settings---page-2) tool.
* **Steps of lumber thickness:** this option allows you to set the thresholds to which the wood thickness value will be rounded in the case of the [r - raw wood](https://github.com/dprojects/Woodworking/tree/master/Docs#main-report) cut list in the "rough" section. The thickness of each board from the "finish" section will be rounded to the first encountered threshold from this list that corresponds to the value of the global variable `MagicPanels.gSawmillSteps`.
* **Add lumber allowance everywhere:** this option allows you to include these additional dimensions of lumber for the sawmill in standard cut lists. In this case, the cut lists will be created with these additional dimensions, i.e., the "rough" section itself.

### Settings - page 3

* **Front inside - thickness:** allows you to set default thickness for fronts inside.
* **Front inside - offset from left:** allows you to set default left gap for fronts inside.
* **Front inside - offset from right:** allows you to set default right gap for fronts inside.
* **Front inside - offset from bottom:** allows you to set default bottom gap for fronts inside.
* **Front inside - offset from top:** allows you to set default top gap for fronts inside.
* **Front outside - thickness:** allows you to set default thickness for fronts outside.
* **Front outside - overlap from left:** allows you to set default left overlap for fronts outside.
* **Front outside - overlap from right:** allows you to set default right overlap for fronts outside.
* **Front outside - overlap from bottom:** allows you to set default bottom overlap for fronts outside.
* **Front outside - overlap from top:** allows you to set default top overlap for fronts outside.
* **Shelf - thickness:** allows you to set default thickness for shelves.
* **Shelf - the offset from sides:** this option allows you for global setting of the gap on both sides for the inner shelf, making it easier to insert the shelf. In practice, cutting inner shelves to within a millimeter is quite risky. A 1-millimeter error in cutting can also occur, causing the shelf to fit too tightly, and hammering the shelf into place can cause unnecessary stress on the joints, screws, dowels, and weaken the structure. Not everyone has a table saw or jigsaw capable of cutting 2 mm of chipboard, or sanding such an oversized wooden shelf can be time-consuming. Additionally, shelf supports pins are approximately 5 mm length outside, with 1 mm being the flange. Therefore, maintaining a 1 mm gap on each side allows the shelf to rest on the entire support rather than just the flange, as can be the case with shelves that are too tight.
...
* **Dowel penetration:** initial size inside board for the dowels in [magicDowels](#magicdowels) tool.
...
* **Spike inside hole:** how the spike for hole should be:
...
* **Pocket hole rotation:** pocket hole drill bit angle to the face in [magicDriller](#magicdriller) tool. You can also try value with minus.
* **Pocket hole penetration:** this is space above the face for the pocket hole drill bit spike in [magicDriller](#magicdriller) tool. You can also try value with minus.
...
* **transport space limit:** it also displays information about the longest and shortest edges. Of course the thickness is not calculated here. This allows you to estimate the maximum space you will need in your car trunk to transport all the boards, as well as whether you will exceed the minimum cutting size. In Poland, chipboard cutting services typically require a minimum cutting size of approximately 100 mm, so for safety reasons, they usually do not cut 50 mm pieces. In this case, for the `q - for cut service` and `g - wood type` and `m - material description` report types, an additional column will also be displayed specifying whether a given dimension group falls within the transport space limit. The transport space limit can be set in the [magicSettings](#magicsettings) tool in the options `Wdth of transport space` and `Length of transport space`.
...
* **Preselection mode:**
  * `Turn ON` button allows you to start preselection mode and also exit selection mode. Here the measurements are not parametric.
  * `Turn OFF` button allows you to start selection mode and also exit preselection mode. Here the measurements are parametric.
...
* **Adjust penetration:** With this option you can change the sign for the sink. Sometimes it solves the problem with correct positioning and further adjust is not needed.
...
* **Offset along X axis:** The `-` and `+` buttons allows you to move an object along the X coordinate axis.
* **Offset along Y axis:** The `-` and `+` buttons allows you to move an object along the Y coordinate axis.
* **Offset along Z axis:** The `-` and `+` buttons allows you to move an object along the Z coordinate axis.
