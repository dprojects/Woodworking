...
* **Wood weight calculation:** this option allows you to choose the weight calculation method. Currently, the following calculation methods are supported:
  * `kg per area in m^2`: the `Wood weight` will be considered as kilograms and will be multiplied by the area in meters.
  * `kg per volume in m^3`: the `Wood weight` will be considered as kilograms and will be multiplied by the volume in meters.
  * `kg per wood piece`: the `Wood weight` will be considered as kilograms and will be multiplied by the wood piece.
  * `pounds per area in ft^2`: the `Wood weight` will be considered as pounds and will be multiplied by the area in feet.
  * `pounds per cubic foot`: the `Wood weight` will be considered as pounds and will be multiplied by the volume in feet.
  * `pounds per cubic inch`: the `Wood weight` will be considered as pounds and will be multiplied by the volume in inches.
  * `pounds per board foot`: the `Wood weight` will be considered as pounds and will be multiplied by the board foot.
  * `pounds per wood piece`: the `Wood weight` will be considered as pounds and will be multiplied by the wood piece.
...
* **Wood price calculation:** this option allows you to choose the price calculation method. Currently, the following calculation methods are supported:
  * `price per area in m^2`: the `Wood price` will be multiplied by the area in meters. This is the method of displaying prices in stores in Poland, so it is the default option.
  * `price per volume in m^3`: the `Wood price` will be multiplied by the volume in meters.
  * `price per wood piece`: the `Wood price` will be multiplied by the wood piece.
  * `price per area in ft^2`: the `Wood price` will be multiplied by the area in feet.
  * `price per cubic foot`: the `Wood price` will be multiplied by the volume in feet.
  * `price per cubic inch`: the `Wood price` will be multiplied by the volume in inches.
  * `price per board foot`: the `Wood price` will be multiplied by the board foot.
...
> [!TIP]
> If you want to reset settings to defaults, 
> please remove parameters in `User parameter:BaseApp/Preferences/Woodworking`
> via FreeCAD parameter editor `Tools -> Edit parameters... -> Preferences -> Woodworking`.
