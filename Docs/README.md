...
* **Wood price calculation:** this option allows you to choose the price calculation method. Currently, the following calculation methods are supported:
  * `price per area in m^2`: the price given above will be calculated per square meter of area. This is the method of displaying prices in stores in Poland, so it is the default option.
  * `price per volume in m^3`: when purchasing raw wood at a sawmill, sometimes the price is based on the volume of a given piece of wood, which is why this method of calculating the price is also available.
